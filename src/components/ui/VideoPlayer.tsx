...
const PlayerWrapper = styled.div`
  ...
  @media (max-width: ${({ theme }) => theme.breakpoints.md}) {
    border-radius: 0;
    min-height: 0;
    padding-top: 56.25%; /* Manter 16:9 para mobile também */
  }
`;
...
const PlayerControlsComponent = memo(({
  ...
  // Se for mobile, usar controles adaptados para toque
  if (isMobile) {
    return (
      <MobileControls visible={visible}>
        <PlayerHeader>
...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ 
  ...
  // Handle video ending
  const handleEnded = () => {
    // Simply stop playing when video ends
    setPlaying(false);
  };
